...
;	type.
; CALLING SEQUENCE:
;      MEASURE_NA, data, [NA=, ENA=, FEH=, EFEH=,
;		HINDK2=, SPTYPE=,
;		NITERS=, /ERROR,
;		CCORR_FXN=,
;		/QUIET]
...
;	ccorr_fxn = cross-correlation routine to use ['c_correlate']
;	quiet = limit messages
;
; OUTPUTS:
...
PRO measure_na, data, na=ew, ena=eew, feh=nafeh, efeh=efeh, hindk2=hind, sptype=sp_hind, niters=ni, error=doerrors, ccorr_fxn=ccorr_fxn
...
;     hvec = FLTARR(ni)
  ...
  FOR i=0, ni DO BEGIN
    ...
    ; shift to rest
    ERN_RV, mydata, std[*,*,order], wrange=wrange, rv0=rv0, ccorr_fxn=ccorr_fxn, /quiet
    ...
    ewi = measure_ew(lambda0,flux0,continuum,feature)
